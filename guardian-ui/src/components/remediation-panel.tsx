...
import type { RemediationSuggestion, LocalizedText } from "@/lib/types";
...
/** Pull the current locale's text out of a {zh,en} object, with fallback to the
 *  other language. Strings (legacy cache shape) pass through unchanged. */
function pickLang(v: LocalizedText | undefined, locale: string): string {
  if (!v) return "";
...
export function RemediationPanel({ remediations, highlightSkill, hideSkillName = false }: RemediationPanelProps) {
          ...
          >
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2.5">
                  ...
                  {hideSkillName && (
                    <span className="text-[13px] font-bold text-stone-800 truncate">{pickLang(rem.finding_title, locale)}</span>
                  )}
                  <span className={cn("inline-flex px-2 py-0.5 rounded-md text-[10px] font-bold border shrink-0", severityBg(rem.severity))}>
                    {rem.severity}
                  </span>
                </div>
                {!hideSkillName && (
                  <p className="text-[11px] text-stone-400 mt-0.5 truncate">{pickLang(rem.finding_title, locale)}</p>
                )}
              </div>
              <svg
            ...
            {isExpanded && (
              <div className="border-t border-stone-200 px-5 py-5 space-y-4 bg-stone-50/50">
                <p className="text-[12px] text-stone-500 leading-relaxed">{pickLang(rem.description, locale)}</p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="rounded-xl overflow-hidden border border-red-200">
                    <div className="px-4 py-2 bg-red-50 border-b border-red-200 flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-bold uppercase tracking-wider text-red-700">{t("report.remediation.code_before")}</span>
                    </div>
...
{rem.code_before}
                  ...
                  <div className="rounded-xl overflow-hidden border border-emerald-200">
                    <div className="px-4 py-2 bg-emerald-50 border-b border-emerald-200 flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-bold uppercase tracking-wider text-emerald-700">{t("report.remediation.code_after")}</span>
                    </div>
...
{rem.code_after}
                ...
                <div className="rounded-xl bg-cyan-50 border border-cyan-200 px-5 py-4">
                  <div className="text-[10px] font-bold uppercase tracking-widest text-cyan-700 mb-2 flex items-center gap-1.5">
                    ...
                    {t("report.remediation.explanation")}
                  </div>
                  <p className="text-[12px] text-cyan-800/70 leading-relaxed">{pickLang(rem.explanation, locale)}</p>
                </div>
              </div>
            )}
          </div>
