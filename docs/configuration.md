...
`GUARDIAN_PROVIDER` drives **both Stage A + B (litellm)** and **Stage C (in-container Agent)**.

| Variable | Description |
|---|---|
| `GUARDIAN_PROVIDER` | Provider shortcut. One of: `azure` / `openai` / `anthropic` / `gemini` / `zhipu` / `zai` / `glm` / `minimax` / `minimax-cn` / `minimaxi` / `moonshot` / `kimi` / `qwen` / `dashscope` / `tongyi` / `deepseek` / `doubao` / `ark` / `volcengine` / `xai` / `grok` / `groq` / `openrouter` / `mistral` / `ollama` (see `config.py`) |
| `GUARDIAN_MODEL_ID` | Model name (`gpt-5.4`, `glm-4.6`, `MiniMax-M2.5`, `claude-3-5-sonnet-latest`) |
...
Azure additionally requires `GUARDIAN_API_BASE` & `GUARDIAN_API_VERSION`, for example:
...
If Stage A + B and Stage C need different models or providers, the following variables override the unified config independently:

| Variable | Description |
|---|---|
| `GUARDIAN_LLM_MODEL` | Override the model used for Stage A + B |
| `GUARDIAN_LLM_BASE_URL` | Override the API base URL for Stage A + B |
| `GUARDIAN_DOCKER_MODEL` | Override the model used for Stage C (in-sandbox Agent) |
| `GUARDIAN_DOCKER_API_URL` | Override the API base URL for Stage C |

These take priority over the auto-derived values from `GUARDIAN_PROVIDER`. When not set, both stages share the unified configuration.

For example, use GPT for Stage A + B evaluation and Claude for the Stage C sandbox Agent:

```bash
# Unified provider (base config)
...
| `GUARDIAN_PROVIDER` | `GUARDIAN_MODEL_ID` example |
|---|---|
| `azure` | `gpt-5.4` |
| `openai` | `gpt-4o-mini` |
