...
def _build_analyzers(policy: ScanPolicy, args: argparse.Namespace, status: Callable[[str], None]) -> list:
    ...
    analyzers = build_analyzers(
        ...
        use_cloud_defense=getattr(args, "use_cloud_defense", False),
        cloud_defense_api_key=getattr(args, "cloud_defense_api_key", None),
        cloud_defense_api_url=getattr(args, "cloud_defense_api_url", None),
    ...
    for a in analyzers:
        ...
        elif name == "cloud_defense":
            status("Using Cloud Defense analyzer")
        elif name == "trigger":
...
def _configure_taxonomy_and_threat_mapping(args: argparse.Namespace, status: Callable[[str], None]) -> None:
    ...
    from ..threats.fangcun_ai_taxonomy import reload_taxonomy
...
def list_analyzers_command(_args: argparse.Namespace) -> int:
    ...
    entries = [
        ...
        (
            ...
            True,
            "Available (optional)",
            "FangcunGuard Cloud Defense cloud-based threat detection",
            "--use-cloud-defense --cloud-defense-api-key KEY",
        ),
        (
            "llm_analyzer",
            LLM_AVAILABLE,
...
def _add_common_scan_flags(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--use-cloud-defense", action="store_true", help="Enable Cloud Defense analyzer (requires API key)")
    parser.add_argument("--cloud-defense-api-key", help="Cloud Defense API key (or set CLOUD_DEFENSE_API_KEY)")
    parser.add_argument("--cloud-defense-api-url", help="Cloud Defense API URL (optional, defaults to US region)")
...
Examples:
  ...
  skill-scanner list-analyzers
    ...
    scan_all_p.add_argument(
        "--workers",
        type=int,
        default=1,
        metavar="N",
        help="Number of parallel workers for scanning skills (default: 1, serial). Use 4-8 to speed up LLM-heavy scans.",
    )
