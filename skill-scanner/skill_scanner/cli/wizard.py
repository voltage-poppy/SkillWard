...
_ENV_KEYS = {
    ...
    "CLOUD_DEFENSE_API_KEY": "FangcunGuard Cloud Defense",
}
...
def _ask_analyzers(env: dict) -> dict[str, bool]:
    ...
    aid_set = env["keys"].get("CLOUD_DEFENSE_API_KEY", False)
    aid_tag = "[green]\u2714 key set[/]" if aid_set else "[dim]\u2718 key not set[/]"
    analyzers["use_cloud_defense"] = Confirm.ask(
        f"  FangcunGuard Cloud Defense  {aid_tag}",
        default=aid_set,
    )
...
def _build_command(
    ...
    *,
    ...
    if analyzers:
        ...
        if analyzers.get("use_cloud_defense"):
            cmd.append("--use-cloud-defense")
