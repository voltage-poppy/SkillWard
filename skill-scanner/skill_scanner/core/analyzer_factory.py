...
def build_core_analyzers(
    ...
    *,
    ...
    Args:
        ...
        use_parallel_static_engines: When True, prepend the Phase1ParallelGroup
            (PatternAnalyzer + StructureValidatorAnalyzer + CapabilityRiskAnalyzer)
            and configure StaticAnalyzer to skip the text-pattern checks that are
            now handled by the parallel engines.
    ...
    _effective_parallel = use_parallel_static_engines or getattr(policy.analyzers, "use_parallel_static_engines", False)

    if _effective_parallel:
        try:
            from .analyzers.phase1_parallel_group import Phase1ParallelGroup

            analyzers.append(Phase1ParallelGroup(policy=policy))
        except (ImportError, Exception) as exc:
            logger.warning("Could not load Phase1ParallelGroup: %s", exc)

    if policy.analyzers.static:
        analyzers.append(
            StaticAnalyzer(
                custom_yara_rules_path=custom_yara_rules_path,
                _skip_text_pattern_checks=_effective_parallel,
                policy=policy,
            )
        )
...
def build_analyzers(
    ...
    *,
    ...
    if use_cloud_defense:
        try:
            from .analyzers.cloud_defense_analyzer import CloudDefenseAnalyzer

            key = cloud_defense_api_key or os.getenv("CLOUD_DEFENSE_API_KEY")
            url = cloud_defense_api_url or os.getenv("CLOUD_DEFENSE_API_URL")
            if not key:
                logger.warning("Cloud Defense requested but no API key.  Set CLOUD_DEFENSE_API_KEY or pass cloud_defense_api_key")
            else:
                analyzers.append(CloudDefenseAnalyzer(api_key=key, api_url=url))
        except (ImportError, ValueError, TypeError) as exc:
            logger.warning("Could not load Cloud Defense analyzer: %s", exc)
