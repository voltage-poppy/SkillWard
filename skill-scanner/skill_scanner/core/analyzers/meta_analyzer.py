...
class MetaAnalyzer(BaseAnalyzer):
    ...
    def __init__(
        ...
        # Initialize ProviderConfig and LLMRequestHandler for unified LLM access
        # This ensures GPT-5 models use the Responses API via aresponses()
...
Respond with a JSON object following the schema in the system prompt."""
    ...
    async def _make_llm_request(self, system_prompt: str, user_prompt: str) -> str:
        """Make a request to the LLM API using the unified request handler.

        Uses LLMRequestHandler which automatically routes GPT-5 models to the
        Responses API (via aresponses) and other models to Chat Completions API.
        """
        messages = [
        ...
        return await self.request_handler.make_request(messages, context="meta-analysis")
