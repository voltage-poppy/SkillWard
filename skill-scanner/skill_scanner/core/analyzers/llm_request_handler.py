...
try:
    ...
    from litellm import aresponses as _aresponses
...
class LLMRequestHandler:
    ...
    def _load_response_schema(self) -> dict[str, Any] | None:
        ...
        try:
            ...
            if schema_path.exists():
                ...
                try:
                    from ...threats.fangcun_ai_taxonomy import VALID_AITECH_CODES
    ...
    async def make_request(self, messages: list[dict[str, str]], context: str = "") -> str:
        ...
        elif self.provider_config.is_gpt5 and aresponses is not None:
            # GPT-5 models require the Responses API (not Chat Completions API)
            return await self._make_responses_api_request(messages, context)
        else:
    ...
    async def _make_responses_api_request(self, messages: list[dict[str, str]], context: str) -> str:
        """Make request using LiteLLM's aresponses for GPT-5 models (Responses API).

        The Responses API is the new endpoint for GPT-5 models that provides
        enhanced reasoning capabilities. LiteLLM's aresponses() provides a
        unified interface similar to acompletion().

        See: https://docs.litellm.ai/docs/responses
        """
        last_exception = None
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                # Responses API supports json_schema format
                if self.response_schema:
                    request_params["response_format"] = {
                        "type": "json_schema",
                        "json_schema": {
                ...
                # Use aresponses for GPT-5 models
                response = await aresponses(**request_params, drop_params=True)

                # Extract content from Responses API response format
                # The response structure is similar but may have different attributes
                if hasattr(response, "output_text") and response.output_text:
                    content: str = response.output_text
                elif hasattr(response, "choices") and response.choices:
                    content = response.choices[0].message.content or ""
                elif hasattr(response, "output") and response.output:
                    # Responses API may return output directly
                    if isinstance(response.output, str):
                        content = response.output
                    elif isinstance(response.output, list):
                        # Output might be a list of content blocks
                        text_parts = []
                ...
                if any(
                    ...
                    if attempt < self.max_retries:
                        delay = (2**attempt) * self.rate_limit_delay
                        logger.warning(
                            "Rate limit hit for %s (Responses API), retrying in %ss (attempt %d/%d)",
                            context,
                            delay,
                            attempt + 1,
                            self.max_retries + 1,
                ...
                # For other errors, don't retry
                logger.error("Responses API error for %s: %s", context, e)
                break
