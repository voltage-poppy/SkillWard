...
"""
Cloud Defense API analyzer for agent skills security scanning.

Integrates with FangcunGuard Cloud Defense API (https://api.clouddefense.fangcunguard.com) for:
...
# Cloud Defense API endpoint - FangcunGuard Cloud Defense Inspect API
CLOUD_DEFENSE_API_URL = "https://us.api.inspect.clouddefense.security.fangcunguard.com/api/v1"

# Default enabled rules for Cloud Defense API
...
class CloudDefenseAnalyzer(BaseAnalyzer):
    """
    Analyzer that uses FangcunGuard Cloud Defense API for threat detection.

    Sends skill content (prompts, markdown, code) to Cloud Defense API
    ...
    **Important**: The "Code Detection" rule is automatically excluded when
    ...
    Example:
        >>> # Basic usage
        >>> analyzer = CloudDefenseAnalyzer(api_key="your-api-key")
    ...
    def __init__(
        ...
        """
        Initialize Cloud Defense API analyzer.

        Args:
            api_key: Cloud Defense API key (or set CLOUD_DEFENSE_API_KEY env var)
            api_url: Custom API URL (defaults to https://api.clouddefense.fangcunguard.com/api/v1)
        ...
        """
        super().__init__("cloud_defense_analyzer")

        if not HTTPX_AVAILABLE:
            raise ImportError("httpx is required for Cloud Defense analyzer. Install with: pip install httpx")

        # Get API key from parameter or environment
        self.api_key = api_key or os.getenv("CLOUD_DEFENSE_API_KEY")
        if not self.api_key:
            raise ValueError(
                "Cloud Defense API key required. Set CLOUD_DEFENSE_API_KEY environment variable or pass api_key parameter."
            )

        self.api_url = api_url or os.getenv("CLOUD_DEFENSE_API_URL", CLOUD_DEFENSE_API_URL)
    ...
    def _get_client(self) -> httpx.AsyncClient:
        ...
        if self._client is None:
            ...
            self._client = httpx.AsyncClient(
                ...
                headers={
                    "X-FangcunGuard-Cloud-Defense-Key": self.api_key,
    ...
    def analyze(self, skill: Skill) -> list[Finding]:
        """
        Analyze skill using Cloud Defense API (sync wrapper).
    ...
    async def analyze_async(self, skill: Skill) -> list[Finding]:
        """
        Analyze skill using Cloud Defense API (async).
        ...
        try:
            ...
            logger.error("Cloud Defense API analysis failed for %s: %s", skill.name, e)
    ...
    async def _analyze_prompt_content(
        ...
        """
        Analyze prompt/instruction content via Cloud Defense API.
        ...
        try:
            ...
            # Call Cloud Defense API - chat inspection endpoint
            response = await self._make_api_request(endpoint="/inspect/chat", payload=payload)

            if response:
                # Process Cloud Defense response format:
                # {
                ...
                for classification in classifications:
                    if classification and classification != "NONE_VIOLATION":
                        ...
                        findings.append(
                            Finding(
                                id=self._generate_id(f"CLOUD_DEF_{classification}", file_path),
                                rule_id=f"CLOUD_DEF_{classification}",
                                category=self._map_violation_category(classification),
                                severity=severity,
                                title=f"{classification.replace('_', ' ').title()} detected",
                                description=f"Cloud Defense detected {classification.replace('_', ' ').lower()} in {file_path}",
                                file_path=file_path,
                                remediation="Review and address the security concern flagged by Cloud Defense",
                                analyzer="cloud_defense",
                                metadata={
                ...
                for rule in rules:
                    ...
                    findings.append(
                        Finding(
                            id=self._generate_id(f"CLOUD_DEF_RULE_{rule_name}", file_path),
                            rule_id=f"CLOUD_DEF_RULE_{rule_name.upper().replace(' ', '_')}",
                            ...
                            description=f"Cloud Defense rule '{rule_name}' detected {rule_classification.replace('_', ' ').lower()}",
                            file_path=file_path,
                            remediation=f"Address the {rule_name.lower()} issue detected by Cloud Defense",
                            analyzer="cloud_defense",
                            metadata={
                                "rule_name": rule_name,
                ...
                if action == "block" and not is_safe:
                    ...
                    if not findings:
                        findings.append(
                            Finding(
                                id=self._generate_id("CLOUD_DEF_BLOCKED", file_path),
                                rule_id="CLOUD_DEF_BLOCKED",
                                category=ThreatCategory.PROMPT_INJECTION,
                                severity=Severity.HIGH,
                                title="Content blocked by Cloud Defense",
                                description=f"Cloud Defense blocked content in {file_path} as potentially malicious",
                                file_path=file_path,
                                analyzer="cloud_defense",
                                metadata={
                                    "action": action,
            ...
            logger.error("Cloud Defense prompt analysis failed for %s: %s", file_path, e)
    ...
    async def _analyze_code_content(
        ...
        """
        Analyze code content via Cloud Defense API.
        ...
        try:
            ...
            # Call Cloud Defense API - chat inspection endpoint for code
            response = await self._make_api_request(endpoint="/inspect/chat", payload=payload)

            if response:
                # Process Cloud Defense response (same format as prompt inspection)
                is_safe = response.get("is_safe", True)
                classifications = response.get("classifications", [])
                rules = response.get("rules", [])
                action = response.get("action", "").lower()
                ...
                for classification in classifications:
                    if classification and classification != "NONE_VIOLATION":
                        ...
                        findings.append(
                            Finding(
                                id=self._generate_id(f"CLOUD_DEF_CODE_{classification}", file_path),
                                rule_id=f"CLOUD_DEF_CODE_{classification}",
                                category=self._map_violation_category(classification),
                                severity=severity,
                                title=f"Code {classification.replace('_', ' ').lower()} detected",
                                description=f"Cloud Defense detected {classification.replace('_', ' ').lower()} in {language} code",
                                file_path=file_path,
                                remediation="Review and fix the code issue flagged by Cloud Defense",
                                analyzer="cloud_defense",
                                metadata={
                                    "classification": classification,
                                    "language": language,
                ...
                for rule in rules:
                    ...
                    findings.append(
                        Finding(
                            id=self._generate_id(f"CLOUD_DEF_CODE_RULE_{rule_name}", file_path),
                            rule_id=f"CLOUD_DEF_CODE_RULE_{rule_name.upper().replace(' ', '_')}",
                            category=self._map_violation_category(rule_classification),
                            severity=self._map_classification_to_severity(rule_classification),
                            title=f"Code rule triggered: {rule_name}",
                            description=f"Cloud Defense rule '{rule_name}' detected issue in {language} code",
                            file_path=file_path,
                ...
                if action == "block" and not is_safe and not findings:
                    findings.append(
                        Finding(
                            id=self._generate_id("CLOUD_DEF_CODE_BLOCKED", file_path),
                            rule_id="CLOUD_DEF_CODE_BLOCKED",
                            category=ThreatCategory.MALWARE,
                            severity=Severity.HIGH,
                            title="Code blocked by Cloud Defense",
                            description=f"Cloud Defense blocked {language} code in {file_path} as potentially malicious",
                            file_path=file_path,
                            analyzer="cloud_defense",
                            metadata={
                                "action": action,
            ...
            logger.error("Cloud Defense code analysis failed for %s: %s", file_path, e)
    ...
    async def _make_api_request(
        ...
        """
        Make request to Cloud Defense API with retry logic.
                ...
                if response.status_code == 200:
                    ...
                    try:
                        ...
                        if (
                            ...
                            logger.warning(
                                "Cloud Defense API key has pre-configured rules, retrying without enabled_rules config..."
                            )
                    ...
                    # Generic 400 error
                    logger.error("Cloud Defense API error: %s - %s", response.status_code, response.text)
                    return None
                elif response.status_code == 429:
                    # Rate limited - wait and retry
                    delay = (2**attempt) * 1.0
                    logger.warning("Cloud Defense API rate limited, retrying in %ds...", delay)
                    await asyncio.sleep(delay)
                    continue
                elif response.status_code == 401:
                    raise ValueError("Invalid Cloud Defense API key")
                elif response.status_code == 403:
                    raise ValueError("Cloud Defense API access denied - check permissions")
                else:
                    logger.error("Cloud Defense API error: %s - %s", response.status_code, response.text)
                    return None

            except httpx.TimeoutException:
                last_exception = TimeoutError(f"Cloud Defense API timeout after {self.timeout}s")
        ...
        if last_exception:
            logger.error(
                "Cloud Defense API request failed after %d attempts: %s",
                self.max_retries,
                last_exception,
            )
    ...
    def _convert_api_violation_to_finding(
        ...
        """Convert Cloud Defense API violation to Finding object."""
        try:
            ...
            return Finding(
                id=self._generate_id(f"CLOUD_DEF_{violation_type}", file_path),
                rule_id=f"CLOUD_DEF_{violation_type}",
                category=category,
                severity=severity,
                title=violation.get("title", f"Cloud Defense detected: {violation_type.replace('_', ' ').lower()}"),
            ...
            logger.warning("Failed to convert Cloud Defense violation: %s", e)
            return None

    def _map_violation_severity(self, severity_str: str) -> Severity:
        """Map Cloud Defense severity string to Severity enum."""
        severity_map = {
            ...
            "NONE_SEVERITY": Severity.MEDIUM,  # Default for Cloud Defense
        }
    ...
    def _map_classification_to_severity(self, classification: str) -> Severity:
        """Map Cloud Defense classification to severity level."""
    ...
    def _map_violation_category(self, violation_type: str) -> ThreatCategory:
        """Map Cloud Defense violation type to ThreatCategory."""
    ...
    def _convert_api_threat_to_finding(
        ...
        """Convert Cloud Defense API threat to Finding object (legacy support)."""
    ...
    def _convert_api_vulnerability_to_finding(
        ...
        """Convert Cloud Defense API vulnerability to Finding object."""
        try:
            ...
            return Finding(
                id=self._generate_id(f"CLOUD_DEF_VULN_{vuln_type}", f"{file_path}_{vuln.get('line', 0)}"),
                rule_id=f"CLOUD_DEF_VULN_{vuln_type}",
                category=category,
                severity=severity,
            ...
            logger.warning("Failed to convert Cloud Defense vulnerability: %s", e)
            return None

    def _map_threat_type_to_category(self, threat_type: str) -> ThreatCategory:
        """Map Cloud Defense threat type to ThreatCategory."""
        mapping = {
            "PROMPT_INJECTION": ThreatCategory.PROMPT_INJECTION,
