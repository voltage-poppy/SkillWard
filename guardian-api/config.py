...
class GuardianSettings:
    ...
    llm_model: str = ""                   # filled by GUARDIAN_LLM_MODEL or PROVIDER_TABLE derivation
    llm_api_key: str = ""
    llm_base_url: str = ""
    llm_api_version: str = ""             # Azure-specific; filled by GUARDIAN_API_VERSION
...
_ENV_MAP = {
    "GUARDIAN_LLM_PROVIDER": "llm_provider",
    "GUARDIAN_LLM_MODEL": "llm_model",
    "GUARDIAN_LLM_API_KEY": "llm_api_key",
    "GUARDIAN_LLM_BASE_URL": "llm_base_url",
    "GUARDIAN_LLM_API_VERSION": "llm_api_version",
    "GUARDIAN_DOCKER_IMAGE": "docker_image",
    "GUARDIAN_DOCKER_MODEL": "docker_model",
    "GUARDIAN_DOCKER_API_URL": "docker_api_url",
    "GUARDIAN_DOCKER_API_KEY": "docker_api_key",
    "GUARDIAN_DOCKER_VERSION": "docker_api_version",
...
def load_settings() -> GuardianSettings:
    ...
    # 3. Shared fallbacks: GUARDIAN_API_KEY / GUARDIAN_API_VERSION fill in
    #    both llm_* and docker_* fields when they are still empty.
    shared_key = os.environ.get("GUARDIAN_API_KEY")
    if shared_key:
        if not s.llm_api_key:
            s.llm_api_key = shared_key
        if not s.docker_api_key:
            s.docker_api_key = shared_key
    shared_ver = os.environ.get("GUARDIAN_API_VERSION")
    if shared_ver:
    ...
    # 4. Provider-table derivation: GUARDIAN_PROVIDER + GUARDIAN_MODEL_ID
    #    derive llm_model / docker_model / *_base_url for both stages.
    #    Only fills fields still empty — explicit GUARDIAN_LLM_MODEL /
    #    GUARDIAN_DOCKER_MODEL etc. always win as escape hatches.
    provider = (os.environ.get("GUARDIAN_PROVIDER") or "").strip().lower()
    model_id = (os.environ.get("GUARDIAN_MODEL_ID") or "").strip()
    api_base = (os.environ.get("GUARDIAN_API_BASE") or "").strip()

    if provider:
        provider = PROVIDER_ALIASES.get(provider, provider)  # normalize aliases
        spec = PROVIDER_TABLE.get(provider)
        if spec is None:
            known = ", ".join(sorted(PROVIDER_TABLE.keys()))
            raise RuntimeError(
                f"Unknown GUARDIAN_PROVIDER={provider!r}. "
                f"Known providers: {known}. "
                f"Or set GUARDIAN_LLM_MODEL/GUARDIAN_DOCKER_MODEL directly."
            )

        base = api_base or spec.get("default_base", "")
        profile = provider  # the @profile segment is just an auth-profile name
        ...
        if model_id and not s.docker_model:
            s.docker_model = f"{spec['openclaw_provider']}/{model_id}@{profile}"
        if base and not s.docker_api_url:
            s.docker_api_url = base + spec.get("docker_base_suffix", "")

        # Azure-style providers require an api_version
        if spec.get("needs_version") and not (s.llm_api_version or s.docker_api_version):
            raise RuntimeError(
                f"Provider {provider!r} requires an API version. "
                f"Set GUARDIAN_API_VERSION (e.g. 2025-04-01-preview)."
            )
...
_REQUIRED_FIELDS = {
    ...
    "docker_model":   "GUARDIAN_DOCKER_MODEL",
    "docker_api_url": "GUARDIAN_DOCKER_API_URL",
    "docker_api_key": "GUARDIAN_DOCKER_API_KEY",
}
